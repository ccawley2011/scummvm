...
static const char *addrToName(const void *ptr) {
	if (((uintptr)ptr > 0x02000000 && (uintptr)ptr < 0x02400000) ||
	    ((uintptr)ptr > (u32)__itcm_start && (uintptr)ptr < (u32)(__itcm_start + 32768))) {
		// TODO: Get the symbol name from the main executable? */
		return nullptr;
	} else {
		for (int i = 0; i < PLUGIN_TYPE_MAX; i++) {
			const PluginList &pl = PluginMan.getPlugins((PluginType)i);
			for (Plugin *p : pl) {
				ELFPlugin *elf = dynamic_cast<ELFPlugin *>(p);
				if (elf) {
					const char *name = elf->addrToName(ptr);
					if (name) {
						return name;
					}
				}
...
//---------------------------------------------------------------------------------
static void guruMeditationDump() {
//---------------------------------------------------------------------------------
#if 0
	...
	symbolName = addrToName((const void *)exceptionRegisters[14]);
	if (symbolName) {
		char buffer[1024];
		size_t len = 1024;
		int status;

		char *demangledName = abi::__cxa_demangle(symbolName, buffer, &len, &status);
		if (demangledName) {
			iprintf("  pc: %08lX addr: %08lX\n sym: %s\n\n",codeAddress,exceptionAddress,demangledName);
		} else {
			iprintf("  pc: %08lX addr: %08lX\n sym: %s\n\n",codeAddress,exceptionAddress,symbolName);
		}
	} else {
...
//---------------------------------------------------------------------------------
	setExceptionHandler(scummvmHandler) ;
}

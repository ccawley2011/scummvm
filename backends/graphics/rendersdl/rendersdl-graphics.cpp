...
bool RenderSdlGraphicsManager::saveScreenshot(const Common::String &filename) const {
	...
	data.create(_windowWidth, _windowHeight, format);
...
void RenderSdlGraphicsManager::setMouseCursor(const void *buf, uint w, uint h, int hotspotX, int hotspotY, uint32 keyColor, bool dontScale, const Graphics::PixelFormat *format, const byte *mask) {
	...
	if (keycolorChanged) {
		SDL_SetColorKey(_mouseOrigSurface, SDL_TRUE, _mouseKeyColor);
	}
...
void RenderSdlGraphicsManager::blitCursor() {
	...
	SDL_SetColorKey(_mouseSurface, SDL_TRUE, _mouseKeyColor);
...
void RenderSdlGraphicsManager::displayMessageOnOSD(const Common::U32String &msg) {
	...
	// Lock the surface
	if (SDL_LockSurface(osdMessageSurface))

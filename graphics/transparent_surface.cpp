...
TransparentSurface *TransparentSurface::convertTo(const PixelFormat &dstFormat, const byte *palette) const {
	...
	if (format.bytesPerPixel == 0 || format.bytesPerPixel > 4)
		error("TransparentSurface::convertTo(): Can only convert from 1Bpp, 2Bpp, 3Bpp, and 4Bpp");

	if (dstFormat.bytesPerPixel < 2 || dstFormat.bytesPerPixel > 4)
		error("TransparentSurface::convertTo(): Can only convert to 2Bpp, 3Bpp and 4Bpp");

...
static const Tucker::TuckerGameDescription tuckerGameDescriptions[] = {
	{
		{
			"tucker",
			"",
	...
	},
	{
		...
		Tucker::kGameFlagDemo
	},
	AD_TABLE_END_MARKER
};

static const Tucker::TuckerGameDescription tuckerDemoGameDescription = {
	{
		"tucker",
	...
	Tucker::kGameFlagDemo | Tucker::kGameFlagIntroOnly
};

class TuckerMetaEngineDetection : public AdvancedMetaEngineDetection<Tucker::TuckerGameDescription> {
public:
			...
			if (d->isDirectory() && d->getName().equalsIgnoreCase("audio") && d->getChildren(audiofslist, Common::FSNode::kListFilesOnly)) {
				for (Common::FSList::const_iterator f = audiofslist.begin(); f != audiofslist.end(); ++f) {
					if (!f->isDirectory() && f->getName().equalsIgnoreCase("demorolc.raw")) {
						return ADDetectedGame(&tuckerDemoGameDescription.desc);
					}
				}
			}
		}

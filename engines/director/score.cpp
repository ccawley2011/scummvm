...
void Score::screenShot() {
	...
	Common::String prefix = Common::String::format("%s%s", currentPath.c_str(), Common::punycode_encodefilename(_movie->getMacName().decode()).c_str());
...
void Score::loadActions(Common::SeekableReadStreamEndian &stream) {
	...
	for (auto &j : _actions) {
		if (!j._value.empty()) {
			...
			_movie->getMainLingoArch()->addCode(j._value.decode(), kScoreScript, j._key, nullptr, kLPPTrimGarbage);
